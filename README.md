...
| Variable | Purpose |
| --- | --- |
| `NEXT_PUBLIC_SITE_URL` | Canonical origin for canonical tags, `sitemap.xml` and absolute `og:image` URLs. Use the brand host; `*.vercel.app` values are ignored. Local unset → `http://localhost:3000`; otherwise → `https://heirloomcribsandmore.com`. |
