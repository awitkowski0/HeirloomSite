...
const V3 = path.join(REVIEW, 'v4')
const V3_PROD = path.join(V3, 'products')

const VERSION = {
  id: 'v4',
  task: 'Task 1 — Site Product Audit',
  title: 'Soft-sell voice rewrite — no order policy on PDPs',
...
<footer>${esc(VERSION.id)} · ${esc(VERSION.task)} · <code>review/v4/</code></footer>
<script>
const ROWS = ${dataJson};
...
<meta charset="utf-8"/><meta name="viewport" content="width=device-width, initial-scale=1"/>
<meta http-equiv="refresh" content="0; url=v4/audit.html"/>
...
<p><b>Latest:</b> <a href="v4/audit.html">v4 — Soft-sell voice rewrite</a></p>
...
console.log(JSON.stringify({ version: 'v4', stats, path: path.join(V3, 'audit.html') }, null, 2))
