...
## 2025-06-08 - Fix Missing Capability Token for Order API
**Vulnerability:** The endpoint `api/orders/[paymentIntentId].ts` fetched order details based on the `paymentIntentId` query parameter without authentication. As Stripe Payment Intent IDs are long strings but can potentially be leaked or accessed, relying on them alone as authorization allows Insecure Direct Object References (IDOR), leading to exposed PII (names, addresses, order history).
**Learning:** Never rely solely on object IDs or identifiers (even complex ones) for authorization to access sensitive data unless they act as a one-time secret. Implementing a capability token using an HMAC-SHA256 hash derived from the object ID and a server-side secret provides a robust way to ensure the client requesting access genuinely performed the action that generated the token.
**Prevention:** Always sign critical identifiers using a cryptographically secure hash (HMAC) when passing them to the client and returning them for sensitive object retrieval, and verify this signature via a constant-time comparison (`crypto.timingSafeEqual`) to mitigate timing side-channel attacks.
