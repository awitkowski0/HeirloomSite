...
## 2026-06-09 - Prevent Order Info Leakage via IDOR
**Vulnerability:** The `/api/orders/[paymentIntentId]` endpoint allowed anyone with a valid `paymentIntentId` to retrieve detailed order information, including Personally Identifiable Information (PII) like names, email addresses, and shipping addresses. This is an Insecure Direct Object Reference (IDOR) vulnerability, as `paymentIntentId`s can be intercepted or predictably guessed in some contexts.
**Learning:** Sensitive user data tied to identifiers must be protected by authentication or an unpredictable authorization token (capability token). Relying solely on the secrecy of an identifier like `paymentIntentId` is insufficient, especially when it is exposed in client-side URLs or logs.
**Prevention:** Implemented an HMAC-SHA256 capability token generated server-side using the `STRIPE_SECRET_KEY` and the `paymentIntentId`. This token is required as a query parameter when fetching order details, cryptographically ensuring that only the user who created the order (and thus received the token during the session) can access it.
