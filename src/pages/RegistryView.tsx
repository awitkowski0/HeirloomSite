        ...
        <section aria-label="Registry items">
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {registry.items.map((item: any, idx: number) => {
              ...
              return (
                <Link key={idx} to={`/product/${encodeURIComponent(pName)}?wood=${encodeURIComponent(item.wood)}&stain=${encodeURIComponent(item.stainName)}`} style={{ textDecoration: 'none', display: 'block', color: 'inherit' }}>
