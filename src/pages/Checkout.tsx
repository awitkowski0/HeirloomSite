      ...
      <div className="grid-layout">
        <div className="product-showcase" style={{ flex: 7 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '48px' }}>
            ...
            {clientSecret && (
              <PaymentSection clientSecret={clientSecret} agreedToTerms={agreedToTerms} token={token} onSuccess={(piId, token) => { clearCart(); navigate(`/order-confirmation/${piId}?token=${encodeURIComponent(token)}`); }} />
            )}
          </div>
        </div>
