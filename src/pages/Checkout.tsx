      ...
      <div className="grid-layout">
        <div className="product-showcase" style={{ flex: 7 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '48px' }}>
            ...
            {clientSecret && token && (
              <PaymentSection clientSecret={clientSecret} agreedToTerms={agreedToTerms} onSuccess={(piId) => { clearCart(); navigate(`/order-confirmation/${piId}?token=${encodeURIComponent(token)}`); }} />
            )}
          </div>
        </div>
