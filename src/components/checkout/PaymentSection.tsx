...
function StripeForm({ onSuccess, token }: { onSuccess: (paymentIntentId: string, token: string) => void, token: string }) {
...
export default function PaymentSection({ clientSecret, agreedToTerms, onSuccess, token }: Props) {
  return (
      ...
      {clientSecret && (
        <div style={{ opacity: agreedToTerms ? 1 : 0.5, pointerEvents: agreedToTerms ? 'auto' : 'none' }}>
          <Elements stripe={stripePromise} options={{ clientSecret, appearance: { theme: 'flat' } }}>
            <StripeForm onSuccess={onSuccess} token={token} />
          </Elements>
        </div>
      )}
    </section>
